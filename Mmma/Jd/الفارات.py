...
@jepthon.ar_cmd(pattern="وقت(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            event,
...
@jepthon.ar_cmd(pattern="زخرفة الصورة(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
...
@jepthon.ar_cmd(pattern="استخدامي$")
