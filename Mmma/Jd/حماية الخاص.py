...
@et_40.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
#ترجمه وكتابة فريق جـيبثون
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
#ترجمه وكتابة فريق جـيبثون
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
#ترجمه وكتابة فريق جـيبثون
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
