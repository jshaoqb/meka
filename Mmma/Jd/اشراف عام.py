#    جميع الحقوق محفوظة كتابة وتعديل  :   @et_40
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @et_40", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@et_40.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(jepthon):
    ...
    try:
        ...
        pass
    if me == user:
       et_40 = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
...
@et_40.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(jepthon):
    dc = razan = jepthon
    i = 0
    sender = await dc.get_sender()
