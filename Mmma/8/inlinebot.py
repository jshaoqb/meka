...
def main_menu():
    text = f"**- [سـورس ميكي](https://t.me/et_40)\
        \n المسـاعد\
        \n\
        \n◽ ميكي لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Jmthon"):
            buttons = [
                (
                    ...
                    Button.url(" et_40 UsᴇʀBoᴛ", "https://t.me/et_40"),
                )
            ]
            ...
            result = builder.article(
                title="© et_40 Help",
                description="Help menu for et_40",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/et_40"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/et_40",
                ),
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
