...
@et_40.bot_cmd(incoming=True, func=lambda e: e.is_private)
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await et_40.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await et_40.tgbot.get_messages(BOTLOG_CHATID, fa_id)
...
@et_40.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@et_40.bot_cmd(incoming=True, func=lambda e: e.is_private)
@et_40.bot_cmd(edited=True, func=lambda e: e.is_private)
